...
from .serializers import (AvatarSerializer, CustomAuthTokenSerializer,
                        ...
                        RecipeCreateUpdateSerializer, RecipeMiniSerializer,
                        RecipeSerializer,
...
def redirect_to_full_link(request, short_code):
    ...
    #     return HttpResponse("Invalid short code or recipe not found", status=404)
...
class UserViewSet(viewsets.ModelViewSet):
    ...
    def set_avatar(self, request):
        ...
        if request.method == 'PUT':
            ...
            return Response({'detail': 'Необходимо предоставить файл для обновления аватара.'}, status=status.HTTP_400_BAD_REQUEST)
    ...
    def set_password(self, request):
        ...
        if not current_password or not new_password:
            return Response({'detail': 'Требуется указать текущий и новый пароль.'}, status=status.HTTP_400_BAD_REQUEST)
    ...
    def subscriptions(self, request):
        ...
        serializer = UserWithRecipesSerializer(paginated_users, many=True, context={'request': request})
    ...
    def subscribe(self, request, pk=None):
        ...
        if request.method == 'POST':
            ...
            if Subscription.objects.filter(user=request.user, following=user_to_action).exists():
            ...
            Subscription.objects.create(user=request.user, following=user_to_action)
            serializer = UserWithRecipesSerializer(user_to_action, context={'request': request})
            return Response(serializer.data, status=status.HTTP_201_CREATED)
        
        subscription = Subscription.objects.filter(user=request.user, following=user_to_action)
...
class RecipeViewSet(viewsets.ModelViewSet):
    ...
    permission_class = permission_classes = [permissions.IsAuthenticatedOrReadOnly]
    ...
    def add_to_favorite(self, request, pk=None):
        '''
        action for add recipe for favorites user's list: POST and DELETE methods
        '''
    ...
    @action(detail=True, url_path='shopping_cart', methods=['delete', 'post'], permission_classes=[permissions.IsAuthenticated])
    def manage_shopping_cart(self, request, pk=None):
        ...
        if request.method == 'POST':
            ...
            if cart_item.exists():
                ...
                return Response({'error': 'Recipe not found in shopping cart'}, status=status.HTTP_400_BAD_REQUEST)

    @action(detail=False, url_path='download_shopping_cart', methods=['get'], permission_classes=[permissions.IsAuthenticated])
    def download_shopping_cart_csv(self, request):
        ...
        for cart_item in user.shopping_cart.all():
            ...
            for recipe_ingredient in recipe.recipeingredient_set.all():
                ingredients[(recipe_ingredient.ingredient.name, recipe_ingredient.ingredient.measurement_unit)] += recipe_ingredient.amount
    ...
    def destroy(self, request, *args, **kwargs):
        ...
        if recipe.author != request.user:
            return Response({'detail': 'У вас нет прав для удаления этого рецепта.'}, status=403)
